"""
Code adapted from https://github.com/atong01/conditional-flow-matching/blob/main/torchcfm/optimal_transport.py
"""
import warnings
...
class OTPlanSampler:
    ...
    def sample_plan_with_indices(self, x0, x1, replace=True):
        r"""Like sample_plan but also returns the indices (i, j) so the caller
        can permute arbitrary conditioning tensors with the same assignment.

        Returns
        -------
        x0[i] : Tensor, shape (bs, *dim)
        x1[j] : Tensor, shape (bs, *dim)
        i : np.ndarray, shape (bs,) — indices into x0
        j : np.ndarray, shape (bs,) — indices into x1
        """
        pi = self.get_map(x0, x1)
        i, j = self.sample_map(pi, x0.shape[0], replace=replace)
